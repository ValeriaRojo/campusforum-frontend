...
export class ValidatorService {
  ...
  public alphanumeric(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).trim();
    if (!s) return false;
   ...
   */
  public wordsES(
    input: Nullable<unknown>,
    options?: { allowHyphen?: boolean; allowApostrophe?: boolean }
  ): boolean {
    const s = this.toStr(input).trim();
    if (!s) return false;
  ...
  public regex(input: Nullable<unknown>, exp: RegExp): boolean {
